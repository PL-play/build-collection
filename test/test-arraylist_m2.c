...
#include <string.h>
DECLARE_ARRAY_LIST(int,M_)
DEFINE_ARRAY_LIST(-1,M_)
...
void test_append_arraylist(void) {
  ...
  for (i = 0; i < 10000; ++i) {
    assert(M_append_arraylist(arraylist, -1) == M_ARRAYLIST_SUCCESS);
  }
...
void test_prepend_arraylist(void) {
  ...
  for (i = 0; i < 10000; ++i) {
    assert(M_prepend_arraylist(arraylist, -1) == M_ARRAYLIST_SUCCESS);
  }
